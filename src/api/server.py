...
from src.schemas import EnrichRequest, EnrichResponse, ErrorResponse, Enrichment
...
app = FastAPI()
logger = logging.getLogger("api.api_server")
...
async def enrich_alert(request: Request):
    try:
        ...
        logger.warning("ABOUT TO CALL query_ollama")
        enriched = query_ollama(alert)
        logger.warning("RETURNED FROM query_ollama")
        es_doc = {
